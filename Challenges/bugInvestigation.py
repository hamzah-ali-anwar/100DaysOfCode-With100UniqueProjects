...
class CreateUserProfile:
    ...
    def before_create(email):
        try:
            # Fetch social media profiles from the third-party API
            profiles = SocialMediaProfiles.fetch_social_profiles(email)

            # Handle cases where the API fails or returns an invalid response
            if not profiles:
                logging.error(f"Failed to fetch social media profiles for {email}")
                return False  # Fail gracefully

            # Save the profiles
            result = SocialMediaProfiles.save_profiles(profiles)

            # Ensure 'success' key exists in the response
            return result.get('success', False)

        except Exception as e:
            logging.exception(f"Error in before_create for {email}: {str(e)}")
            return False  # Prevent breaking user creation
